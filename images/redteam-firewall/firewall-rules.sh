...
iptables -A FORWARD -s ${INTERNET_NET} -d ${JENKINS} -p tcp --dport 8080 -j ACCEPT
...
# Jenkins can reach fileserver SMB (deploy artifacts)
iptables -A FORWARD -s ${JENKINS} -d ${FILESERVER} -p tcp --dport 445 -j ACCEPT
